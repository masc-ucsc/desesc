...
protected:
 ...
 public:
    void setupFULoad(const std::string &clusterName) {
      ...
      ful  = new FULoad(iLALU_LD, cls, aGen, lsq, ss, pref, scb, lsdelay, l, global_mem_sys_p0, size,  id, "specld");
    
    }
    Dinst *createLDInstAddr(Addr_t addr) {
      auto  *st_inst = Dinst::create(Instruction(iLALU_LD, LREG_R1, LREG_R2, LREG_R3, LREG_R4)
                                    ,0xdeaddead // pc
                                    ,addr
                                    ,0
                                    ,true);
