...
Gmemory_system *global_mem_sys_p0 = nullptr;
...
void initialize() {
  ...
  if (!pluggedin) {
    ...
    global_mem_sys_p0 = new Memory_system(0);
    pluggedin = true;
  }
}
...
Addr_t sb_calc_line(Addr_t addr) {
  return sb->calc_line(addr);
}
Addr_t sb_calc_offset(Addr_t addr) {
  return sb->calc_offset(addr);
}
...
TEST_F(Store_buffer_test, store_buf_line_is_load_forward) {
  ...
  sbline.init(this->get_line_size(), st_addr_line);
